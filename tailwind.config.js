...
const { slate, gray, neutral, stone, red, blue, green, yellow, ...otherColors } = require('tailwindcss/colors');
const spacing = {};

const extraSpacing = {};

for (let i = 11; i <= 100; i++) {
  extraSpacing[i] = `${i * 0.25}rem`;  // Continua padrão 0.25rem = 4px
}
...
module.exports = {
  ...
  safelist: [
    ...
    { pattern: /^apexcharts-.*$/ },
    { pattern: /^leaflet-.*$/ },
    // cores: bg, text, border, hover
    { pattern: /^(bg|text|border|hover:bg|hover:text|hover:border)-(red|blue|green|gray|yellow|lime|sky|pink|purple|indigo|emerald|teal|cyan|amber|stone|slate|neutral|zinc|fuchsia|violet|rose)-(100|200|300|400|500|600|700|800|900)$/ },
    // espaçamentos: margem, padding
    { pattern: /^(m|mt|mr|mb|ml|mx|my|p|pt|pr|pb|pl|px|py)-[0-9]+$/ },
    // grid e flex
    { pattern: /^(grid-cols|col-span|row-span|gap|gap-x|gap-y)-[0-9]+$/ },
    { pattern: /^(flex|justify|items|content)-(start|end|center|between|around|evenly)$/ },
    // rounded
    ...
    // position e z-index
    { pattern: /^(absolute|relative|fixed|sticky|z)-[0-9]+$/ },
    // transition
    { pattern: /^transition(-(none|all|colors|opacity|shadow|transform))?$/ },
    // opacity
    { pattern: /^opacity-[0-9]+$/ },
    // Cores padrão Tailwind com hover
    {
      pattern: /^(bg|text|border)-(red|blue|green|gray|yellow|lime|sky|pink|purple|indigo|emerald|teal|cyan|amber|stone|slate|neutral|zinc|fuchsia|violet|rose)-(100|200|300|400|500|600|700|800|900)$/,
      variants: ['hover', 'focus', 'active'],
    },
    {
      pattern: /^(bg|text|border)-(primary|success|info|danger|warning|dark|light|secondary|brand|white)$/,
      variants: ['hover', 'focus', 'active'],
    },
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        ...
        'dark-inner': 'inset 0 2px 8px rgba(0, 0, 0, 1)',
      },
      fontSize: {
        '4xs': [
          '0.5625rem', 								// 9px
